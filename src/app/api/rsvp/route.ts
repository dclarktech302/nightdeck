...
export async function POST(request: Request) {
  try {
    ...
    // -- UPSERT ATTENDEE --
    // If this email already exists, return the existing row.
    // If not, create a new attendee record.
    // onConflict: 'email' means "if email already exists, update nothing — just return"
    const { data: attendee, error: attendeeError } = await supabase
      .from('attendees')
      .upsert(
        { email: emailLower, first_name: firstName, phone: phone || null, source: source || 'direct' },
    ...
    if (attendeeError || !attendee) {
      console.error('Attendee upsert error:', attendeeError)
      return NextResponse.json(
    ...
    if (existing) {
      // Already registered — return 409 so the form shows the duplicate state
      return NextResponse.json(
    ...
    const { data: rsvp, error: rsvpError } = await supabase
      ...
      .insert({
        ...
        status:      'confirmed',
        // confirmation_code is auto-generated by the database default
      })
